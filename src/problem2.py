...
def problem2a(circle, rectangle, window):
    ...
    yagayeet = rg.Line(rectangle.get_upper_right_corner(), rectangle.get_lower_left_corner())
    yagayeet.arrow = 'last'
    yagayeet.attach_to(window)
    window.render()
    window.get_next_mouse_click()
...
def problem2b(rect, n, delta, win):
    ...
    win.render()
    for k in range(n):
        градславныйлидерсталин = rg.Rectangle(rg.Point(rect.get_lower_left_corner().x - (k * delta), rect.get_lower_left_corner().y + (k * delta)), rg.Point(rect.get_upper_right_corner().x + (k * delta), rect.get_upper_right_corner().y - (k * delta)))
        градславныйлидерсталин.attach_to(win)
        win.render()
