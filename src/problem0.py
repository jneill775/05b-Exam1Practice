...
def problem0c(circle, n, window):
    ...
    for k in range(n + 1):
        generaldouglasmcarthur = rg.Circle(rg.Point(circle.center.x + circle.radius * 2 * k, circle.center.y), circle.radius)
        window.render(0.5)
        if k == 0:
            generaldouglasmcarthur.fill_color = circle.fill_color

        generaldouglasmcarthur.attach_to(window)
